                                ...
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" data-user-id="{{ $user->id }}">
                                    ...
                                    <td class="px-6 py-4">
                                        <a href="{{ route('web.users.show', $user->id) }}"
                                           class="font-medium text-blue-600 dark:text-blue-500 hover:underline mr-2">{{ __('messages.view') }}</a>
                                        <a href="{{ route('web.users.edit', $user->id) }}"
                                           class="font-medium text-blue-600 dark:text-blue-500 hover:underline">{{ __('messages.edit') }}</a>
                                        <button type="button"
                                           class="delete-user-btn font-medium text-red-600 dark:text-red-500 hover:underline ml-4 cursor-pointer"
                                           data-user-id="{{ $user->id }}"
                                           data-delete-url="{{ route('web.admin.users.destroy', ['id' => $user->id]) }}"
                                        >
                                            {{ __('messages.delete') }}
                                        </button>
                                    </td>
