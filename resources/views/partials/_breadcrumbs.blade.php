<!-- resources/views/partials/_breadcrumbs.blade.php -->

@if(isset($breadcrumbs) && count($breadcrumbs) > 0)
    <nav class="flex py-3 px-5 text-gray-700 bg-gray-50 rounded-lg border border-gray-200 mb-4" aria-label="Breadcrumb">
            ...
            @foreach($breadcrumbs as $label => $url)
                <li>
                    <div class="flex items-center">
                        ...
                        @else
                            <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2">{{ $label }}</span>
                        @endif
                    </div>
                </li>
            @endforeach
